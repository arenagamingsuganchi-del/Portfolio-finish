...
export default async function handler(req, res) {
  ...
  try {
    const { workId, emoji } = req.body;

    if (!workId || !emoji) {
      return res.status(400).json({ error: 'workId and emoji are required' });
    }
    ...
    return res.status(200).json({ success: true, reactions: work.reactions });
  } catch (error) {
